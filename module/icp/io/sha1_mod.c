...
sha1_digest_update_uio(SHA1_CTX *sha1_ctx, crypto_data_t *data)
{
	 ...
	 * digested.
	 */
	zuio_nonemptyindex(data->cd_uio, offset, vec_idx);
	if (vec_idx == zuio_iovcnt(data->cd_uio)) {
		/*
	 ...
	 */
	while (vec_idx < zuio_iovcnt(data->cd_uio) && length > 0) {
		cur_len = MIN(zuio_iovlen(data->cd_uio, vec_idx) -
		    offset, length);

		SHA1Update(sha1_ctx,
		    (uint8_t *)zuio_iovbase(data->cd_uio, vec_idx) + offset,
		    cur_len);
	...
	if (vec_idx == zuio_iovcnt(data->cd_uio) && length > 0) {
		/*
    ...
    ulong_t digest_len, uchar_t *digest_scratch)
{
	...
	/* we support only kernel buffer */
	if (zuio_segflg(digest->cd_uio) != UIO_SYSSPACE)
		return (CRYPTO_ARGUMENTS_BAD);
	 ...
	 * be returned.
	 */
	zuio_nonemptyindex(digest->cd_uio, offset, vec_idx);
	if (vec_idx == zuio_iovcnt(digest->cd_uio)) {
		/*
	...
	if (offset + digest_len <=
	    zuio_iovlen(digest->cd_uio, vec_idx)) {
		/*
		...
		if (digest_len != SHA1_DIGEST_LENGTH) {
			...
			bcopy(digest_scratch, (uchar_t *)zuio_iovbase(digest->
			    cd_uio, vec_idx) + offset,
			    digest_len);
		} else {
			SHA1Final((uchar_t *)zuio_iovbase(digest->
			    cd_uio, vec_idx) + offset,
			    sha1_ctx);
		}
	} else {
		...
		while (vec_idx < zuio_iovcnt(digest->cd_uio) && length > 0) {
			cur_len = MIN(zuio_iovlen(digest->cd_uio, vec_idx) -
			    offset, length);
			bcopy(digest_tmp + scratch_offset,
			    zuio_iovbase(digest->cd_uio, vec_idx) + offset,
			    cur_len);
		...
		if (vec_idx == zuio_iovcnt(digest->cd_uio) && length > 0) {
			/*
			 * The end of the specified iovec's was reached but
			 * the length requested could not be processed, i.e.
    ...
    crypto_spi_ctx_template_t ctx_template, crypto_req_handle_t req)
{
	...
	case CRYPTO_DATA_UIO: {
		...
		/* we support only kernel buffer */
		if (zuio_segflg(mac->cd_uio) != UIO_SYSSPACE)
			return (CRYPTO_ARGUMENTS_BAD);

		/* jump to the first iovec containing the expected digest */
		zuio_nonemptyindex(mac->cd_uio, offset, vec_idx);
		if (vec_idx == zuio_iovcnt(mac->cd_uio)) {
			/*
			 * The caller specified an offset that is
			 * larger than the total size of the buffers
			 * it provided.
		...
		while (vec_idx < zuio_iovcnt(mac->cd_uio) && length > 0) {
			cur_len = MIN(zuio_iovlen(mac->cd_uio, vec_idx) -
			    offset, length);

			if (bcmp(digest + scratch_offset,
			    zuio_iovbase(mac->cd_uio, vec_idx) + offset,
			    cur_len) != 0) {
				ret = CRYPTO_INVALID_MAC;
				break;
			}
