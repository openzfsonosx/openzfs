...
skein_digest_update_uio(skein_ctx_t *ctx, const crypto_data_t *data)
{
	...
	if (zuio_segflg(uio) != UIO_SYSSPACE)
	 ...
	 * digested.
	 */
	zuio_nonemptyindex(uio, offset, vec_idx);
	if (vec_idx == zuio_iovcnt(uio)) {
		/*
	 ...
	 */
	while (vec_idx < zuio_iovcnt(uio) && length > 0) {
		cur_len = MIN(zuio_iovlen(uio, vec_idx) - offset, length);
		SKEIN_OP(ctx, Update, (uint8_t *)zuio_iovbase(uio, vec_idx)
		    + offset, cur_len);
		length -= cur_len;
		vec_idx++;
		offset = 0;
	...
	if (vec_idx == zuio_iovcnt(uio) && length > 0) {
		/*
    ...
    crypto_req_handle_t req)
{
	...
	if (offset + CRYPTO_BITS2BYTES(ctx->sc_digest_bitlen) <=
	    zuio_iovlen(uio, vec_idx)) {
		/* The computed digest will fit in the current iovec. */
		SKEIN_OP(ctx, Final,
		    (uchar_t *)zuio_iovbase(uio, vec_idx) + offset);
	} else {
		uint8_t *digest_tmp;
		...
		while (vec_idx < zuio_iovcnt(uio) && length > 0) {
			cur_len = MIN(zuio_iovlen(uio, vec_idx) - offset,
			    length);
			bcopy(digest_tmp + scratch_offset,
			    zuio_iovbase(uio, vec_idx) + offset, cur_len);
