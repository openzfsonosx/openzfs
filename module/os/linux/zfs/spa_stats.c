...
spa_tx_assign_update(kstat_t *ksp, int rw)
{
	...
	if (rw == KSTAT_WRITE) {
		for (i = 0; i < shk->count; i++)
			((kstat_named_t *)shk->_private)[i].value.ui64 = 0;
	}

	for (i = shk->count; i > 0; i--)
		if (((kstat_named_t *)shk->_private)[i-1].value.ui64 != 0)
			break;
...
spa_tx_assign_init(spa_t *spa)
{
	...
	shk->_private = kmem_alloc(shk->size, KM_SLEEP);

	name = kmem_asprintf("zfs/%s", spa_name(spa));

	for (i = 0; i < shk->count; i++) {
		ks = &((kstat_named_t *)shk->_private)[i];
...
spa_tx_assign_destroy(spa_t *spa)
{
	...
	kmem_free(shk->_private, shk->size);
...
spa_tx_assign_add_nsecs(spa_t *spa, uint64_t nsecs)
{
	...
	atomic_inc_64(&((kstat_named_t *)shk->_private)[idx].value.ui64);
}
