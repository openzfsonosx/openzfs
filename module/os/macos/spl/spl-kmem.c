...
#define	MEMLEAK_LOW   16  /* Inclusive */
#define	MEMLEAK_HIGH  32  /* Exclusive */
#define	MEMLEAK_MAX_STACK 11
...
static inline void leak_add(void *ptr, size_t size)
{
	...
	if (size >= MEMLEAK_LOW && size < MEMLEAK_HIGH) {
		...
		if (ml) {
			printf("SPL: leak %p exists ml->size %llu, called %lu."
			    "Existing leak stack: \n",
			    ptr, ml->size, size);
			dump_stack(ml->stack);
	...
	if (size >= MEMLEAK_LOW && size < MEMLEAK_HIGH) {
		...
		if (ml) {
			if (ml->size != size) {
				uintptr_t stack[MEMLEAK_MAX_STACK];
				getpcstack(stack, MEMLEAK_MAX_STACK);
				printf("SPL: leak free of %p different size "
				    "%llu != %lu from:\n",
			...
			uintptr_t stack[MEMLEAK_MAX_STACK];
			getpcstack(stack, MEMLEAK_MAX_STACK);
			printf("SPL: leak free of %p:%lu not in memleak_tree\n",
			    buf, size);
			dump_stack(stack);
		}
	}
...
spl_kmem_fini(void)
{
	...
	memleak_t *ml;
	void *cookie = NULL;
	unsigned long nodes = avl_numnodes(&memleak_tree);
	...
	if (nodes > 0)
		panic("Dumped leaks - attach to resolv symbols");
#endif
