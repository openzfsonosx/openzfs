...
int (*REAL_kauth_cred_getgroups)(kauth_cred_t _cred, gid_t *_groups,
    int *_groupcount) = NULL;
...
#if 0
int (*REAL_fp_drop)(struct proc *p, int fd, struct fileproc *fp,
    int locked) = NULL;
int (*REAL_fp_drop_written)(struct proc *p, int fd, struct fileproc *fp,
    int locked) = NULL;
int (*REAL_fp_lookup)(struct proc *p, int fd, struct fileproc **resultfp,
    int locked) = NULL;
int (*REAL_fo_read)(struct fileproc *fp, struct uio *uio, int flags,
    vfs_context_t ctx) = NULL;
int (*REAL_fo_write)(struct fileproc *fp, struct uio *uio, int flags,
    vfs_context_t ctx) = NULL;
#endif
...
macho_find_load_command(mach_header_t *header, uint32_t cmd)
{
	...
	for (uint_t i = 0; i < header->ncmds; i++, cur += lc->cmdsize) {
		lc = (load_cmd*)cur;
		if (lc->cmd == cmd) {
			return (lc);
		}
...
segment_command_t *
macho_find_segment(mach_header_t *header, const char *segname)
{
	segment_command_t *cur_seg_cmd;
	vm_address_t cur = (vm_address_t)header + sizeof (mach_header_t);
	for (uint_t i = 0; i < header->ncmds;
...
section_t *
macho_find_section(mach_header_t *header, const char *segname,
    const char *secname)
{
	segment_command_t *cur_seg_cmd;
	vm_address_t cur = (vm_address_t)header + sizeof (mach_header_t);
...
void *
macho_find_symbol(mach_header_t *header, const char *name)
{
	...
	uintptr_t linkedit_base =
	    (uintptr_t)vmaddr_slide + linkedit_segment->vmaddr -
...
spl_loadsymbols(void)
{
	...
	vm_offset_t func_address =
	    (vm_offset_t)vm_kernel_unslide_or_perm_external;
	...
	LOAD_SYMBOL(REAL_vm_page_speculative_count,
